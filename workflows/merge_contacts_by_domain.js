...
exports.main = async (event, callback) => {
  ...
  *****/
  ...
  try {
    ...
    const email_domain_search = await searchRecord([{
                "propertyName": "hs_email_domain",
                "value": hs_email_domain,
                "operator": "EQ"
              },
              {
	...
	try {
      // MERGE BASED ON EMAIL DOMAIN
      if (email_domain_search.length >= 2) {
        let primary_record;
        let objects_to_merge = [];
        for (const record of email_domain_search) {
